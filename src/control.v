 ...
 *
 *   RUN:   [13] dense_mode (0 = 1:2 sparse K=6, 1 = dense E2M1 K=3)
 *          Clears accumulators, streams the skewed wavefront for
 *          2N+1 = 7 cycles (3 weight-code steps either way — sparse
 *          covers two contraction steps per code, dense covers one).
 *
 ...
 * the reference — one sparse pair step has the same schedule as one
